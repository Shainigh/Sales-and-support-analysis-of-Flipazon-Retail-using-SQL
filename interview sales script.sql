...
select table_name, count(*) as no_of_records
from information_schema.tables
where table_schema = 'interview'
group by table_name;
...
select * from orders;
select count(ORD_NUM) as number_of_orders, sum(ORD_AMOUNT) as total_revenue, avg(ORD_AMOUNT) as avg_order_value from orders;
...
SELECT AGENT_NAME, count(GRADE) AS count_of_high_grade_customers
FROM customer join agents on agents.AGENT_CODE = customer.AGENT_CODE
GROUP BY AGENT_NAME
having count(GRADE) =5
ORDER BY count_of_high_grade_customers DESC;
...
select CUST_CITY, sum(ORD_AMOUNT) as total_order_amount from customer inner join orders on customer.CUST_CODE = orders.CUST_CODE
group by CUST_CITY
...
select count(CUST_CODE) AS total_no_of_cust from customer;

/*QUESTION-2 Identify the top-spending customers based on their total order value.*/

select CUST_NAME, sum(ORD_AMOUNT) as total_order_value from customer inner join orders on customer.CUST_CODE= orders.CUST_CODE
group by CUST_NAME
order by total_order_value desc;

*** /*QUESTION-3 Analyse customer retention by calculating the percentage of repeat customers.*/ ***

select count(*) as total_cust_count,
sum(case when repeat_count> 1 then 1 else 0 end) as repeat_customers,
(sum(case when repeat_count> 1 then 1 else 0 end)/ count(*)) * 100 as prcnt_rpt_cust
from
(
	select CUST_CODE, count(CUST_CODE) as repeat_count 
	from customer 
	group by CUST_CODE
) as x;
...
select CUST_NAME, CUST_COUNTRY, max(OUTSTANDING_AMT) as max_outstand from customer
group by CUST_COUNTRY , CUST_NAME 
order by max_outstand desc;

/*QUESTION-5 Write a SQL query to calculate the percentage of customers in each grade category (1 to 5).*/

select GRADE , count(GRADE) as no_of_cust_per_grade,
(count(GRADE)/(select count(*) from customer)) *100 as percnt_cust   /*count(grade)--> no of customer per grade, select count(*) from customer --> no of total customer in the table*/
from customer
group by GRADE
order by GRADE;
...
select AGENT_NAME, max(ORD_AMOUNT) as cnt_ord_amt from agents JOIN orders on agents.AGENT_CODE = orders.AGENT_CODE
group by AGENT_NAME
order by cnt_ord_amt desc
limit 5;      /* to check*/
...
The company wants to analyse the performance of agents based on the number of orders they have handled.
 ...
 select AGENT_NAME, count(ORD_NUM) as no_of_ord_agnt,
 row_number() over(order by count(ORD_NUM) desc) as agent_rank
 from agents join orders on agents.AGENT_CODE = orders.AGENT_CODE
 group by AGENT_NAME;
...
PART-1 If the average advance amount collected is less than 750, there is no change in commission.
PART-2 If the average advance amount collected is between 750 and 1000 (inclusive), the new commission will be 1.5 times the old commission.
PART-3 If the average advance amount collected is more than 1000, the new commission will be 2 times the old commission*/ ***

/*ALTER table agents
ADD COLUMN updated_commission decimal;

UPDATE agents

 SET updated_commission = case 
	when select (avg(ADVANCE_AMOUNT) < 750 then updated_commission = "no change" end,
    when avg(ADVANCE_AMOUNT) > 750 and =< 1000 then updated_commission = "COMMISSION * 1.5" end,
    when avg(ADVANCE_AMOUNT) > 1000 then updated_commission = "COMMISSION * 2" end
    from agents join orders on agents.AGENT_CODE = orders.AGENT_CODE*/
...
DROP COLUMN IF EXISTS avg_rcv_amt;
...
set avg_rcv_amt = (
select avg(RECEIVE_AMT) from customer
where CUST_COUNTRY = c.CUST_COUNTRY
);
select *, avg_rcv_amt from customer;
